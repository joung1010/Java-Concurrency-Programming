...
## 완료 상태처리 및 확인  
Java의 `CompletableFuture`는 비동기 작업을 제어하고 완료 상태를 처리할 수 있는 다양한 메서드를 제공합니다.  
  
`complete()`, `completeFuture()`, `completeOnTimeout()`, `completeExceptionally()` 등과 완료 상태를 확인하는 메서드들의 상세한 설명과 사용법입니다.
...
- 비동기 작업을 정상적으로 완료 상태로 변경하고 결과 값을 설정합니다.
- 작업이 이미 완료된 경우, 호출해도 상태는 변경되지 않습니다.
...
public class CompleteExample {
    public static void main(String[] args) {
        ...
        // 결과를 기다리고 출력
        System.out.println(future.join()); // 출력: Task Completed!
    }
}
...
### **1.2 `CompletableFuture.completedFuture()`**

- 결과 값이 이미 정해져 있는 상태로 완료된 `CompletableFuture` 객체를 생성합니다.
- 비동기 작업을 수행하지 않고, 정적인 결과를 반환하는 작업을 생성할 때 유용합니다.
...
public class CompletedFutureExample {
    public static void main(String[] args) {
        ...
        CompletableFuture<String> future = CompletableFuture.completedFuture("Pre-Completed Result");

        // 상태 확인 및 결과 출력
        System.out.println(future.isDone());  // 출력: true
        System.out.println(future.join());    // 출력: Pre-Completed Result
    }
}
...
### **1.3 `completeOnTimeout()`**

- 비동기 작업이 일정 시간 안에 완료되지 않으면, **지정된 값으로 작업을 완료**합니다.
- **Java 9 이상**에서 사용할 수 있습니다.
...
public class CompleteOnTimeoutExample {
    public static void main(String[] args) {
        ...
        }).completeOnTimeout("Default Result", 2, TimeUnit.SECONDS); // 2초 후 대체 값 반환

        // 결과 출력
        System.out.println(future.join()); // 출력: Default Result
    }
}
...
### **1.4 `completeExceptionally()`**

- 작업을 **예외적으로 완료** 상태로 변경합니다.
  - 예외 상태로 완료시키는데 사용되는 메서드로 주어진 예외 객체를 사용하여 비동기 작업을 예외로 처리하는데 사용됩니다.
- 작업이 이미 완료된 경우, 호출해도 상태는 변경되지 않습니다.
  - exceptionally 또는 handle 메서드를 사용하여 예외 처리 작업을 수행할 수 있으며 예외를 처리하고 대체 결과를 반환하거나 다른작업을 수행할 수 있다.
...
- 작업이 완료되었는지 확인합니다.
- 작업이 정상적으로 완료되었든, 예외적으로 완료되었든, 취소되었든 관계없이 **`true`*를 반환합니다.
...
- 작업이 취소되었는지 확인합니다.
- **`cancel(true)`** 메서드를 호출하여 작업을 취소한 경우에만 **`true`*를 반환합니다.
...
public class IsCancelledExample {
    public static void main(String[] args) {
        ...
        System.out.println(future.isCancelled()); // 출력: true
        System.out.println(future.isDone());      // 출력: true
    }
}
...
### **2.3 `isCompletedExceptionally()`**

- 작업이 **예외적으로 완료되었는지** 확인합니다.
- **`completeExceptionally()`** 메서드로 작업을 종료한 경우에만 **`true`*를 반환합니다.
...
public class IsCompletedExceptionallyExample {
    public static void main(String[] args) {
        ...
        // 작업을 예외적으로 완료
        future.completeExceptionally(new RuntimeException("Error occurred"));

        System.out.println(future.isCompletedExceptionally()); // 출력: true
    }
}
...
| **메서드** | **설명** | **상태 변경 여부** |
| --- | --- | --- |
| **`complete(value)`** | 작업을 강제로 정상 완료 상태로 설정하고 결과 값을 반환 | 완료됨 |
| **`CompletableFuture.completedFuture(value)`** | 이미 완료된 상태의 `CompletableFuture` 객체를 생성 | 완료 상태로 반환 |
| **`completeOnTimeout(value)`** | 지정된 시간 안에 완료되지 않으면 대체 값으로 강제로 완료 | 완료됨 |
| **`completeExceptionally(ex)`** | 작업을 강제로 예외 상태로 완료 | 완료됨 |
| **`isDone()`** | 작업이 완료되었는지 확인 (정상/예외/취소 포함) | 상태 유지 |
| **`isCancelled()`** | 작업이 취소되었는지 확인 | 상태 유지 |
| **`isCompletedExceptionally()`** | 작업이 예외적으로 완료되었는지 확인 | 상태 유지 |

---

## **4. 모든 메서드 조합 예제**

아래는 모든 주요 메서드와 상태 확인 메서드를 조합한 예제입니다.

```java

import java.util.concurrent.CompletableFuture;

public class CompletableFutureAllMethodsExample {
    public static void main(String[] args) {
        CompletableFuture<String> future = new CompletableFuture<>();
        ...
        // 2. Attempt to complete exceptionally (ignored as already completed)
        if (!future.isDone()) {
            future.completeExceptionally(new RuntimeException("Error"));
        }

        // 3. Check states
        System.out.println("Is done: " + future.isDone()); // 출력: Is done: true
        System.out.println("Is cancelled: " + future.isCancelled()); // 출력: Is cancelled: false
        System.out.println("Is completed exceptionally: " + future.isCompletedExceptionally()); // 출력: Is completed exceptionally: false

        // 4. Retrieve result
        System.out.println("Result: " + future.join()); // 출력: Success
    }
}
...
---
위 메서드들은 비동기 작업의 다양한 상태와 완료 방식을 제어할 수 있도록 설계되어 있어, 필요한 상황에 따라 적합한 메서드를 선택해 사용할 수 있습니다.

## **비동기 대기 및 취소 처리**

비동기 처리에서 작업 완료를 기다리거나, 필요에 따라 작업을 취소하는 메서드로 **`get()`**, **`join()`**, **`cancel()`**이 사용됩니다.

이들은 **Java의 `CompletableFuture`** 및 관련 비동기 처리에서 자주 사용되는 메서드입니다.

---

### **1. `get()`**

`get()` 메서드는 **작업이 완료될 때까지 대기**한 후, 작업 결과를 반환합니다.

작업 중 예외가 발생하면, 해당 예외를 **`ExecutionException`**으로 감싸 반환하며, **`InterruptedException`**이 발생할 가능성도 있습니다.
...
1. **동기적 대기**
  - 작업이 완료될 때까지 호출한 스레드가 블로킹됩니다.
  - 결과가 반환될 때까지 기다리며, 작업이 완료되면 값을 반환합니다.
2. **예외 처리**
  - 작업 도중 예외가 발생하면 **`ExecutionException`*으로 감싸서 던집니다.
  - 대기 중 스레드가 인터럽트되면 **`InterruptedException`*이 발생합니다.
3. **시간 초과 지원**
  - `get(long timeout, TimeUnit unit)`를 통해 대기 시간을 제한할 수 있습니다.
...
public class GetExample {
    public static void main(String[] args) {
        ...
        try {
            ...
            System.out.println("Result: " + result); // 출력: Result: Result
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
...
`join()`은 **`get()`과 비슷한 메서드**로, 작업이 완료될 때까지 대기한 후 결과를 반환합니다.

**다른 점은 `InterruptedException`을 발생시키지 않고, unchecked 예외로 처리**한다는 것입니다.
...
1. **동기적 대기**
  - 작업 완료까지 호출한 스레드가 블로킹됩니다.
2. **예외 처리**
  - 작업 중 예외가 발생하면 **`CompletionException`*으로 감싸 unchecked 예외로 던집니다.
  - **`InterruptedException`은 발생하지 않으며**, 작업 중 인터럽트가 발생해도 예외를 던지지 않고 계속 실행됩니다.
3. **사용 편의성**
  - `get()`보다 간단하게 사용할 수 있으며, 예외 처리가 간소화됩니다.
...
- *`join()`*의 내부 로직은 아래와 같습니다.
- 내부적으로 **`get()`*을 호출하지만, 예외를 unchecked 예외인 **`CompletionException`*으로 감싸 처리합니다.
...
public T join() {
    try {
        ...
        // get()에서 발생한 예외를 CompletionException으로 감싸 unchecked로 처리
...
`cancel()`은 **비동기 작업을 취소**합니다.

작업이 취소되면 이후 작업은 수행되지 않으며, **`CancellationException`**을 발생시킵니다.
...
1. **작업 중단**
  - 아직 완료되지 않은 작업을 중단합니다.
  - 작업이 실행 중이라도 강제로 중단하지는 않으며, **취소 상태로 표시**만 합니다.
2. **취소 상태 확인**
  - *`isCancelled()`*로 작업이 취소되었는지 확인할 수 있습니다.
3. **취소와 예외 처리**
  - 취소된 작업에 대해 결과를 요청하면 **`CancellationException`*이 발생합니다.
...
public class CancelExample {
    public static void main(String[] args) {
        ...
        // 취소 상태 확인
        System.out.println("Is Cancelled: " + future.isCancelled()); // 출력: Is Cancelled: true
        System.out.println("Is Done: " + future.isDone());           // 출력: Is Done: true
        ...
        try {
            ...
            System.out.println("Exception: " + e.getMessage()); // 출력: Exception: java.util.concurrent.CancellationException
        }
    }
}
...
**`cancel()`** 메서드의 내부 동작은 다음과 같습니다:

1. 작업이 취소 가능한 상태인지 확인합니다.
2. 작업을 **취소 상태로 표시**하며, 이후 호출에서 취소 상태를 감지할 수 있습니다.
3. `mayInterruptIfRunning` 값에 따라 스레드를 중단할 수도 있습니다.

```java

public boolean cancel(boolean mayInterruptIfRunning) {
    // 상태를 '취소(CANCELLED)'로 설정
    if (complete(new AltResult(new CancellationException()))) {
...
| **메서드** | **설명** | **예외 처리** | **상태 변경 여부** |
| --- | --- | --- | --- |
| **`get()`** | 동기적으로 작업 완료를 기다리고 결과를 반환. 시간 초과 설정 가능. | `InterruptedException`, `ExecutionException`, `TimeoutException` | 상태 유지 |
| **`join()`** | 동기적으로 작업 완료를 기다리고 결과를 반환. unchecked 예외로 처리. | `CompletionException` | 상태 유지 |
| **`cancel()`** | 작업을 취소하고 이후 호출에서 `CancellationException` 발생. | `CancellationException` | 작업 취소 상태로 변경 |

---

## **5. 요약 및 활용**

- `get()`은 작업의 결과를 기다릴 때 예외 처리가 필요할 경우 사용합니다.
- `join()`은 간편한 예외 처리를 제공하며, 더 간단하게 사용할 수 있습니다.
- `cancel()`은 필요 시 비동기 작업을 중단하고 취소 상태를 설정할 수 있습니다.


## 내부 구현 코드
`get()` 메서드는 **`Unsafe`** 객체와 **`waiters`(대기열)**를 통해 작업 완료 여부를 확인하고, 완료되지 않은 경우 대기열에 스레드를 추가하여 블로킹 상태로 전환합니다.
...
public V get() throws InterruptedException, ExecutionException {
    Object r = result; // 결과 상태 확인
    if (r == null) {   // 작업이 완료되지 않았다면
        r = waitingGet(false, 0L); // 대기 및 블로킹
    }
    return reportGet(r); // 결과 반환 또는 예외 처리
}
...
- **`result` 필드**는 작업의 현재 상태를 나타냅니다.
  - **`null`**: 작업이 아직 완료되지 않음.
  - **`AltResult(null)`**: 정상적으로 완료된 상태.
  - **`AltResult(Throwable)`**: 작업 중 예외가 발생한 상태.

```java

Object r = result; // 현재 상태 확인

```

---

### **2단계: 대기 및 블로킹 (`waitingGet`)**

- 작업이 아직 완료되지 않았다면 **`waitingGet`** 메서드를 호출하여 호출한 스레드를 블로킹합니다.
- 대기 중인 스레드들은 **`waiters`** 큐에 추가됩니다.
- 작업이 완료되면 대기 중인 스레드들을 깨워줍니다.

### 1. 전체 구현
```java

private Object waitingGet(boolean interruptible) {
  if (interruptible && Thread.interrupted()) // 1. 인터럽트 확인
    return null;

  Signaller q = null; // 2. 대기 객체 초기화
  boolean queued = false; // 대기열 등록 여부
  Object r; // 결과 저장 변수

  while ((r = result) == null) { // 3. 작업 완료 여부 확인
    if (q == null) { // 3.1 Signaller 생성
      q = new Signaller(interruptible, 0L, 0L);
      if (Thread.currentThread() instanceof ForkJoinWorkerThread)
        ForkJoinPool.helpAsyncBlocker(defaultExecutor(), q); // ForkJoinPool 도움 요청
    } else if (!queued) { // 3.2 대기열에 추가
      queued = tryPushStack(q);
    } else if (interruptible && q.interrupted) { // 3.3 인터럽트된 경우 처리
      q.thread = null;
      cleanStack();
      return null;
    } else { // 3.4 대기 상태로 전환
      try {
        ForkJoinPool.managedBlock(q); // ForkJoinPool 대기 처리
      } catch (InterruptedException ie) { // 이론적으로 발생하지 않음
        q.interrupted = true;
      }
    }
  }

  if (q != null) { // 4. Signaller 정리
    q.thread = null;
    if (q.interrupted)
      Thread.currentThread().interrupt();
  }

  postComplete(); // 5. 후속 작업 처리
  return r; // 6. 결과 반환
}
...
- **Signaller**:
  - 대기 중인 스레드를 관리하기 위한 객체로, 스레드를 **대기열(stack)**에 추가하거나 대기 상태로 전환하는 역할을 합니다.
...
- **설명**:
  - 호출한 스레드가 인터럽트 상태인 경우 즉시 **`null`*을 반환합니다.
  - *`Thread.interrupted()`*는 현재 스레드의 인터럽트 상태를 확인하고, 해당 상태를 **초기화**(클리어)합니다.
- **의미**:
  - `interruptible`이 **`true`*이고 스레드가 인터럽트되었으면 대기 없이 반환합니다.
...
- **설명**:
  - `Signaller`는 대기 중인 스레드를 관리하는 내부 객체입니다.
  - `queued`는 현재 스레드가 대기열에 추가되었는지 여부를 나타냅니다.
...
- **설명**:
  - `result` 필드는 작업의 현재 상태를 나타냅니다:
    - **`null`**: 작업이 완료되지 않음.
    - **`AltResult(null)`**: 정상적으로 완료된 상태.
    - **`AltResult(Throwable)`**: 작업 중 예외가 발생한 상태.
  - 결과가 **`null`*인 경우, 작업이 완료되지 않았으므로 대기 상태로 전환됩니다.
...
- **Signaller 생성**:
  - `Signaller`는 현재 스레드를 대기열에 추가하거나 대기 상태로 전환하기 위해 사용됩니다.
  - `interruptible`: 대기 중 인터럽트를 처리할지 여부.
  - `0L, 0L`: 타임아웃 관련 값(현재는 사용하지 않음).
- **ForkJoinPool 도움 요청**:
  - 현재 스레드가 `ForkJoinWorkerThread`인 경우, ForkJoinPool에게 작업 처리 도움을 요청합니다.
  - **`helpAsyncBlocker`**:
    - 대기 상태로 전환된 스레드를 관리하며, 다른 비동기 작업을 병렬로 처리할 수 있도록 지원합니다.
...
- **설명**:
  - 대기열에 `Signaller` 객체를 추가합니다.
  - *`tryPushStack(q)`*는 스레드-안전하게 `stack`에 `q`를 추가합니다.
- **동작 원리**:
  - `stack` 필드는 대기 중인 스레드들의 연결 리스트입니다.
  - `tryPushStack` 메서드는 **CAS(Compare-And-Swap)**를 사용해 `stack`의 맨 위에 새로운 `Signaller`를 추가합니다.
...
- **설명**:
  - `interruptible`이 **`true`*이고, `q.interrupted` 플래그가 설정된 경우 작업을 중단합니다.
  - 현재 스레드를 `cleanStack()` 메서드를 통해 대기열에서 제거합니다.

---

### **3.4 ForkJoinPool을 사용한 대기**
...
```java
    ...
    try {
        ...
        q.interrupted = true; // 인터럽트 플래그 설정
    }
}
...
- **설명**:
  - `ForkJoinPool.managedBlock` 메서드를 통해 현재 스레드를 대기 상태로 전환합니다.
  - `Signaller`는 **`ForkJoinPool.ManagedBlocker`** 인터페이스를 구현하고 있어, ForkJoinPool이 스레드 상태를 관리합니다.
- **`managedBlock` 동작**:
  - 현재 스레드를 블로킹 상태로 전환.
  - 필요 시 다른 작업을 처리하기 위해 새로운 스레드를 생성하거나 대체합니다.

---

### **4. 작업 완료 후 Signaller 정리**

```java

if (q != null) {
    q.thread = null; // 현재 스레드 참조 해제
    if (q.interrupted)
...
- **설명**:
  - 작업이 완료되었으므로, `Signaller` 객체를 정리합니다.
  - `q.thread = null`: 현재 스레드 참조를 해제하여 메모리 누수를 방지.
  - `q.interrupted`가 설정된 경우, **스레드 인터럽트 상태를 복원**합니다.
...
- **설명**:
  - 작업이 완료된 후 후속 작업(체이닝된 비동기 작업)을 실행합니다.
  - 대기열에 등록된 다른 스레드들을 깨우고, 연결된 `CompletableFuture`를 실행합니다.
...
- **설명**:
  - 작업이 완료된 경우, `result`에 저장된 값을 반환합니다.---
  

### **3단계: 결과 반환 및 예외 처리**

- 작업 완료 상태를 반환하거나, 작업 중 발생한 예외를 처리합니다.

```java

private V reportGet(Object r) throws ExecutionException {
    if (r instanceof AltResult) {
        Throwable x = ((AltResult) r).ex; // 작업 중 발생한 예외
        if (x == null) {
            return null; // 정상 완료된 경우
        }
        if (x instanceof CancellationException) {
            throw (CancellationException) x; // 작업 취소 예외
        }
        throw new ExecutionException(x); // 일반 예외
    }
...
`join()` 메서드는 `get()`과 유사하지만, **`InterruptedException`**을 발생시키지 않고 unchecked 예외로 처리합니다.

---

1. **동기적 대기**:
  - 호출한 스레드를 블로킹하여 작업 완료를 기다립니다.
2. **예외 처리**:
  - 작업 중 예외가 발생하면 **`CompletionException`*으로 감싸서 unchecked 예외로 처리합니다
...
public T join() {
    try {
        return get(); // 내부적으로 get() 호출
    } catch (InterruptedException | ExecutionException ex) {
        throw new CompletionException(ex); // 예외를 CompletionException으로 변환
    }
}
...
1. **`get()` 호출**
  - `join()`은 내부적으로 `get()`을 호출하여 작업을 기다리고 결과를 반환합니다.
2. **예외 변환**
  - `get()`에서 발생하는 **`InterruptedException`*과 **`ExecutionException`*을 unchecked 예외인 **`CompletionException`*으로 변환하여 호출자에게 전달합니다.
  - 이 과정에서 **checked 예외를 간소화**하여 개발자가 편리하게 사용할 수 있습니다.

---

## **3. `cancel()`**

`cancel()` 메서드는 비동기 작업을 **취소 상태로 변경**합니다. 작업이 취소되면 이후 호출에서는 **`CancellationException`**이 발생합니다.
...
1. **취소 처리**:
  - 작업을 취소 상태로 설정하며, 실행 중인 작업에 인터럽트를 요청할 수 있습니다.
2. **상태 확인**:
  - 작업이 취소되었는지 확인하려면 **`isCancelled()`** 메서드를 사용할 수 있습니다.
...
public boolean cancel(boolean mayInterruptIfRunning) {
    if (complete(new AltResult(new CancellationException()))) {
        if (mayInterruptIfRunning) {
            Thread t;
            if ((t = runner) != null) {
                t.interrupt(); // 실행 중인 스레드 인터럽트
            }
        }
        return true; // 성공적으로 취소됨
    }
    return false; // 이미 완료된 경우 취소 실패
}
...
1. **취소 상태 설정**
  - `complete()` 메서드를 호출하여 작업 상태를 **취소 상태로 설정**합니다.
  - **`AltResult`** 객체에 **`CancellationException`*을 저장하여 상태를 업데이트합니다.
2. **스레드 인터럽트**
  - `mayInterruptIfRunning`이 **`true`*인 경우, 실행 중인 스레드(`runner`)에 인터럽트를 요청합니다.
  - 이 요청은 작업을 강제로 중단하지 않으며, 작업 코드가 **`InterruptedException`*을 처리하도록 유도합니다.
3. **결과 반환**
  - 작업이 취소되면 **`true`*를 반환하며, 이미 완료된 작업은 취소할 수 없으므로 **`false`*를 반환합니다.

---

### **취소 상태 확인**

취소된 작업은 **`isCancelled()`**를 통해 확인할 수 있습니다.
