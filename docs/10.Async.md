...
private Object waitingGet(boolean interruptible, long nanos) throws InterruptedException {
    Signaller q = null; // 현재 스레드를 대기열에 추가하기 위한 Signaller 객체
    boolean queued = false; // 대기열 추가 여부
    boolean timed = (nanos > 0L); // 시간 초과 여부

    while (result == null) { // 작업이 완료될 때까지 반복
        if (q == null) {
            q = new Signaller(interruptible, nanos); // Signaller 객체 생성
        } else if (!queued) {
            queued = tryPushStack(q); // 대기열에 추가
        } else if (timed) {
            nanos = q.awaitNanos(nanos); // 지정된 시간 동안 대기
        } else {
            q.await(); // 대기열에서 결과를 기다림
        }
    }

    return result; // 작업 완료 시 결과 반환
}
